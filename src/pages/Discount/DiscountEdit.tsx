 ...
 const { i18n } = useTranslation();
  ...
  const[discount, setDiscount]=useState<DiscountRequestDTO>({
     ...
     const [selectedProducts, setSelectedProducts]=useState<MultipleSelectorModel[]>([]);
        ...
        const repodiscountbyId = genericRepository<DiscountDto[], DiscountDto>(`Discounts`);
       const fetchProducts = async (): Promise<ProductResponseDTO[]> => {
            try {
        ...
        const fetchDiscountById = async (products: ProductResponseDTO[]) => {
            if (!numericId) return;
            try {
              const result = await repodiscountbyId.getOne(numericId);
              if (!result.error && result.response) {
                const dto: DiscountDto = result.response;
                const transformed: DiscountRequestDTO = {
                  Id: dto.id,
                  DiscountPercent:dto.discountPercent,
                  StartTime: dto.startTime?.split("T")[0] ?? "",
                  Endtime: dto.endtime?.split("T")[0] ?? "",
                  ProductIds:dto.productDiscounts?.map((p)=> p.productID)|| [],
                  isActive:dto.isActive,                                   
                };        
                setDiscount(transformed);
                console.log("update discount:",transformed)
                const selectedproduct: MultipleSelectorModel[] = products
                                        .filter(sub => transformed.ProductIds?.includes(sub.id))
                                        .map(p => ({
                                            key: p.id.toString(),
                                            value:p.productTranslations
                                                ?.find(t => t.language === i18n.language)
        ...
        useEffect(() => {
         const init = async () => {
            ...
            await fetchDiscountById(products);            
            setLoading(false);
            };
            init();
        ...
        const createOrUpdate = async (values: DiscountRequestDTO) => {
                                const result =  await repo.put(values);
                                if (!result.error) {
